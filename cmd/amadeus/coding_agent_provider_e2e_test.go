	...
	for _, api := range []config.APIMode{config.APIResponses, config.APIChatCompletions} {
		t.Run(string(api), func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
				...
				fmt.Fprint(writer, providerAgentFixture(api, requestIndex))
			}))
			defer server.Close()
...
`, api, dialect, server.URL))
			...
			mutex.Unlock()
			if len(captured) != 3 || stdout.String() != "provider mock complete\n" || len(auditSink.Snapshot()) != 1 {
			...
			if !providerRequestContainsToolResult(api, captured[1]) {
				t.Fatalf("%s second request did not replay tool output: %#v", api, captured[1])
			}
...
func providerRequestContainsToolResult(api config.APIMode, body map[string]any) bool {
	...
	return bytes.Contains(encoded, []byte("provider e2e readme"))
}
