...
func newWebCommand(flags *configFlags, runtime commandRuntime) *cobra.Command {
	command := &cobra.Command{Use: "web", Short: "Diagnose configured web capabilities"}
	command.AddCommand(newWebCheckCommand(flags, runtime))
	return command
}

func newWebCheckCommand(flags *configFlags, runtime commandRuntime) *cobra.Command {
