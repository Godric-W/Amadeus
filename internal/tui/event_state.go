...
type protocolEventState struct {
...
func newProtocolEventState(threadID protocol.ThreadID) *protocolEventState {
	return &protocolEventState{ThreadID: threadID, Active: make(map[protocol.ItemID]protocol.TurnItem)}
}

func (state *protocolEventState) Apply(event protocol.Event) error {
	if state == nil {
...
func (state *protocolEventState) applyDelta(itemID protocol.ItemID, delta string, reset bool) error {
