...
func (model appModel) renderListVisual(visual listVisual, width int) string {
	...
	if strings.TrimSpace(visual.Title) != "" {
		...
		lines = append(lines, truncateLine(header, width))
	}
	if subtitle := strings.TrimSpace(visual.Subtitle); subtitle != "" {
		lines = append(lines, model.palette.dim().Render(truncateLine(subtitle, width)))
		lines = append(lines, "")
	}
	for _, detail := range visual.Details {
		if detail = strings.TrimSpace(detail); detail != "" {
			lines = append(lines, model.palette.plain().Render(truncateLine(detail, width)))
		}
	}
	if visual.InputLabel != "" {
		...
		lines = append(lines, prefix+model.palette.plain().Render(truncateLine(visual.InputValue, valueWidth)))
	}
	if len(visual.Items) == 0 && strings.TrimSpace(visual.EmptyText) != "" {
		lines = append(lines, model.palette.dim().Render(truncateLine(visual.EmptyText, width)))
	}
	nameWidth := 0
...
func (model appModel) renderListVisualItem(item listVisualItem, nameWidth, width int, hideSelectionMarker bool) string {
	prefix := "  "
	if item.Selected && !hideSelectionMarker {
		prefix = model.palette.selection().Render("›") + " "
	}
	name := truncateLine(strings.TrimSpace(item.Name), nameWidth)
