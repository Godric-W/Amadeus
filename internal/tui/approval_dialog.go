...
func (model appModel) handleApprovalKey(key tea.KeyMsg) (tea.Model, tea.Cmd) {
...
func (model appModel) approvalDiffHeight() int {
...
func (model appModel) renderApprovalDialog(width int) string {
	...
	if request.Purpose == policy.ApprovalPurposeCommand {
		...
		for _, line := range strings.Split(command, "\n") {
			rows = append(rows, "   "+model.palette.plain().Render(truncateLine(line, maxInt(12, contentWidth-3))))
		}
		if description := approvalDetailValue(request.Presentation.Details, "Description:"); description != "" {
			rows = append(rows, "   "+model.palette.dim().Render(truncateLine(description, maxInt(12, contentWidth-3))))
		}
		...
		for _, detail := range request.Presentation.Details {
			rows = append(rows, "   "+model.palette.dim().Render(truncateLine(detail, maxInt(12, contentWidth-3))))
		}
	}
	...
	if len(lines) > 0 {
		...
		for _, line := range lines[dialog.diffTop:end] {
			...
			visible = append(visible, style.Render(truncateLine(line, contentWidth)))
		}
