...
func TestAgentRendererRendersCompleteAgentEventSequence(t *testing.T) {
	...
	events := []event.Event{
		...
		event.ToolCallStarted{TurnID: "run-1", CallID: "call-1", ToolName: "read"},
		...
		event.ToolCallCompleted{TurnID: "run-1", CallID: "call-1", ToolName: "read", Success: true, Duration: 12 * time.Millisecond, Summary: "read 10 lines"},
	...
	for _, fragment := range []string{
		...
		"tool: read (call-1) completed in 12ms: read 10 lines",
