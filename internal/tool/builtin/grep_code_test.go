...
func TestGrepCodeFallbackReturnsLineNumbersAndContext(t *testing.T) {
	...
	result, err := grepCode.Execute(context.Background(), json.RawMessage(`{"query":"func Alpha","context":1}`))
	if err != nil {
...
`
	...
	fastResult, err := fast.Execute(context.Background(), input)
	if err != nil {
		t.Fatalf("run rg grep: %v", err)
	}
	fallbackResult, err := fallback.Execute(context.Background(), input)
	if err != nil {
...
func TestGrepCodeFallbackSupportsRegexCaseAndResultLimit(t *testing.T) {
	...
	result, err := grepCode.Execute(context.Background(), json.RawMessage(`{"query":"^todo","regex":true,"case_sensitive":false}`))
