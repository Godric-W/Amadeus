...
func (store terminalFailStore) AppendItems(ctx context.Context, id protocol.ThreadID, turnID protocol.TurnID, items ...rollout.RolloutItem) (threadstore.AppendResult, error) {
	for _, item := range items {
		if isTerminalRolloutItem(item) {
			return threadstore.AppendResult{}, errors.New("terminal persistence failed")
		}
	}
	return store.ThreadStore.AppendItems(ctx, id, turnID, items...)
}

func (store materializeFailStore) Materialize(context.Context, threadstore.CreateInput) (threadstore.AppendResult, error) {
	return threadstore.AppendResult{}, errors.New("materialize failed")
}

func (store turnStartFailStore) AppendItems(ctx context.Context, id protocol.ThreadID, turnID protocol.TurnID, items ...rollout.RolloutItem) (threadstore.AppendResult, error) {
	for _, item := range items {
		if _, ok := item.(rollout.TurnContextItem); ok {
			return threadstore.AppendResult{}, errors.New("turn start append failed")
		}
...
func TestThreadManagerMaterializesOnFirstInput(t *testing.T) {
	...
	listed, err := manager.ListThreads(ctx, threadstore.ListQuery{})
	if err != nil {
		t.Fatal(err)
	}
	if len(listed) != 0 {
	...
	listed, err = manager.ListThreads(ctx, threadstore.ListQuery{CWD: configuration.CWD})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAgentControlSpawnsFullChildSessionAndPersistsNotification(t *testing.T) {
	...
	topLevel, err := manager.ListThreads(ctx, threadstore.ListQuery{})
	if err != nil {
		t.Fatal(err)
	}
	...
	allThreads, err := manager.ListThreads(ctx, threadstore.ListQuery{IncludeSubAgents: true})
	if err != nil {
		t.Fatal(err)
	}
	if len(allThreads) != 2 {
...
func TestResumeRecoversIncompleteToolCall(t *testing.T) {
	...
	live, err := threadstore.NewDraftLiveThread(threadID, store)
	if err != nil {
		t.Fatal(err)
	}
...
func newTestManager(t *testing.T, ctx context.Context, mode string, calls *atomic.Int32) (*ThreadManager, threadstore.ThreadStore) {
	t.Helper()
...
func testConfiguration(t *testing.T) session.Configuration {
	...
	return session.Configuration{Runtime: configured, CWD: filepath.Clean(t.TempDir()), AmadeusRoot: t.TempDir(), Mode: protocol.ModeKindDefault}
}

func testTurnContext(t *testing.T, threadID protocol.ThreadID, turnID protocol.TurnID) session.TurnContext {
	t.Helper()
	configuration := testConfiguration(t)
	return session.TurnContext{
		...
		CWD: configuration.CWD, Mode: protocol.ModeKindDefault,
	}
}
