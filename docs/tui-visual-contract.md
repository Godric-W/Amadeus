...
- Next-turn queue 在 Composer 下方、Footer 上方显示 dim `Queued (n)` 与最多三条单行内容，超出部分显示 `+n more`。Preview 必须按终端宽度截断，Slash Popup 或交互 overlay 激活时隐藏，不进入 main-screen scrollback、Statusline、Rollout 或 Resume replay。
