...
func (model appModel) dispatchCommand(invocation SlashInvocation) (tea.Model, tea.Cmd) {
	...
	case SlashPlan:
		...
		if task == "" {
			if model.pendingMode.Valid() || model.session.mode() == protocol.ModeKindPlan {
				return model, nil
			}
			model.pendingMode = protocol.ModeKindPlan
...
func (model appModel) loadSessions() tea.Cmd {
	return func() tea.Msg {
...
func (model appModel) submitUserMessage(submission UserMessageSubmission) tea.Cmd {
	return func() tea.Msg {
		...
		if err != nil {
			return userMessageRejectedMsg{submission: submission, err: err}
		}
		return userMessageAdmittedMsg{submission: submission, admission: admission}
	}
}

func (model appModel) submitCompact() tea.Cmd {
	return func() tea.Msg {
		if err := model.app.options.Application.SubmitCompact(model.ctx); err != nil {
			return operationFailedMsg{operation: "compact context", err: err}
		}
		return nil
	}
}

func (model appModel) setMode(mode protocol.ModeKind) tea.Cmd {
	return func() tea.Msg {
		if err := model.app.options.Application.SetMode(model.ctx, mode); err != nil {
			return operationFailedMsg{operation: "set collaboration mode", err: err}
		}
		return nil
	}
}

func (model appModel) rename(name string) tea.Cmd {
	return func() tea.Msg {
