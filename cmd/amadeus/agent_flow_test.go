...
func TestPlainInteractiveWorkspaceWriteDoesNotPrompt(t *testing.T) {
	t.Setenv("TERM", "xterm-256color")
	amadeusHome := t.TempDir()
	projectDirectory := t.TempDir()
	writeCodingCommandConfig(t, amadeusHome)
...
func TestInteractivePlanCommandUsesReadOnlyReactor(t *testing.T) {
	amadeusHome := t.TempDir()
	projectDirectory := t.TempDir()
	writeCodingCommandConfig(t, amadeusHome)
	client := &plannedCodingCommandClient{}
	runtime := commandRuntime{
		amadeusRoot:         amadeusHome,
		workingDirectory:    projectDirectory,
		lookupEnv:           emptyEnvLookup,
	...
	if stdout.String() != "1. Inspect repository\n2. Implement changes\n3. Run tests\n" {
		t.Fatalf("unexpected Plan Mode output: %q", stdout.String())
	}
