...
func queueHintProps(width int, plan bool, palette terminalPalette) footerProps {
	...
	return footerProps{
		...
		Palette: palette, LeftPadding: footerLeftPadding, RightPadding: footerRightPadding,
	}
}
...
func TestHasQueueableDraftUsesComposerAndOverlayState(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "idle", mutate: func(model *appModel) { model.running = false }},
		{name: "empty", mutate: func(model *appModel) { model.input.SetValue("  ") }},
		{name: "slash command", mutate: func(model *appModel) { model.input.SetValue("/status") }},
		{name: "invalid slash", mutate: func(model *appModel) { model.input.SetValue("/unknown") }},
		{name: "selection", mutate: func(model *appModel) { model.selection = &selectionOverlay{} }},
		{name: "approval", mutate: func(model *appModel) { model.approvalDialog = &approvalDialog{} }},
		{name: "user input", mutate: func(model *appModel) { model.userInputDialog = &requestUserInputDialog{} }},
		{name: "slash popup", mutate: func(model *appModel) {
...
func TestTUIQueueHintTransitionsAroundTabAndQueuedPreview(t *testing.T) {
