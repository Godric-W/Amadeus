...
func queuedInput(content string, mode protocol.ModeKind, threadID protocol.ThreadID, generation uint64) QueuedUserMessage {
...
func TestNextTurnQueueFIFOAndStartHandshake(t *testing.T) {
	...
	for _, content := range []string{"second", "third"} {
		if err := queue.Enqueue(queuedInput(content, protocol.ModeKindDefault, threadID, 1)); err != nil {
			t.Fatal(err)
		}
	}

	first, ok, err := queue.Begin(threadID, 1, protocol.ModeKindDefault)
	if err != nil || !ok || first.Message.Text != "second" {
		t.Fatalf("first begin = %#v, %v, %v", first, ok, err)
	}
	if _, ok, err := queue.Begin(threadID, 1, protocol.ModeKindDefault); err != nil || ok {
	...
	second, ok, err := queue.Begin(threadID, 1, protocol.ModeKindDefault)
...
func TestNextTurnQueueRejectsStaleAttachmentAndMode(t *testing.T) {
	threadID := testThreadID(1)
	queue := NextTurnQueue{}
	if err := queue.Enqueue(queuedInput("next", protocol.ModeKindPlan, threadID, 2)); err != nil {
		t.Fatal(err)
	}
	if _, ok, err := queue.Begin(testThreadID(2), 2, protocol.ModeKindPlan); ok || !errors.Is(err, errNextTurnQueueAttachmentChanged) {
		t.Fatalf("attachment begin = ok:%v err:%v", ok, err)
	}
	if _, ok, err := queue.Begin(threadID, 2, protocol.ModeKindDefault); ok || !errors.Is(err, errNextTurnQueueModeChanged) {
...
func TestNextTurnQueueRejectsIncompleteInput(t *testing.T) {
	...
	for _, input := range []QueuedUserMessage{
		queuedInput("", protocol.ModeKindDefault, threadID, 1),
		queuedInput("content", "invalid", threadID, 1),
		queuedInput("content", protocol.ModeKindDefault, protocol.ThreadID{}, 1),
		queuedInput("content", protocol.ModeKindDefault, threadID, 0),
	} {
		queue := NextTurnQueue{}
...
func TestTUITabQueuesWithoutRuntimeSubmission(t *testing.T) {
...
func TestTUIEmptyTabDoesNotQueue(t *testing.T) {
	_, model := newTestModel(t, nil)
	model.running = true
...
func TestTUITabKeepsSlashCompletionPriority(t *testing.T) {
	_, model := newTestModel(t, nil)
	model.running = true
...
func TestTUIQueuedInputsDrainOnePerTurn(t *testing.T) {
	_, model := newTestModel(t, nil)
	model.running = true
	for _, content := range []string{"second", "third"} {
	...
	message := command()
	admitted, ok := message.(userMessageAdmittedMsg)
	if !ok {
	...
	if _, ok := command().(userMessageAdmittedMsg); !ok {
...
func TestTUIEnterQueuesBehindPendingStart(t *testing.T) {
	_, model := newTestModel(t, nil)
	if err := model.nextTurnQueue.Enqueue(queuedInput("starting", protocol.ModeKindDefault, model.session.ThreadID, model.session.Generation)); err != nil {
		t.Fatal(err)
	}
...
func TestTUIErrorWaitsForFailedTerminalBeforeQueueDrain(t *testing.T) {
	_, model := newTestModel(t, nil)
	model.running = true
	if err := model.nextTurnQueue.Enqueue(queuedInput("after failure", protocol.ModeKindDefault, model.session.ThreadID, model.session.Generation)); err != nil {
		t.Fatal(err)
	}
...
func TestTUIBlockedAndAbortedTurnsRestoreQueuedInputs(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			model.running = true
			for _, content := range []string{"second", "third"} {
				if err := model.nextTurnQueue.Enqueue(queuedInput(content, protocol.ModeKindDefault, model.session.ThreadID, model.session.Generation)); err != nil {
					t.Fatal(err)
				}
			}
...
func TestTUIQueuedSubmissionFailureRestoresHeadAndKeepsTail(t *testing.T) {
	_, model := newTestModel(t, nil)
	...
	message, ok := command().(userMessageRejectedMsg)
	if !ok {
...
func TestTUIMalformedQueuedAdmissionRestoresHead(t *testing.T) {
	_, model := newTestModel(t, nil)
	model.running = true
	if err := model.nextTurnQueue.Enqueue(queuedInput("retry malformed", protocol.ModeKindDefault, model.session.ThreadID, model.session.Generation)); err != nil {
		t.Fatal(err)
	}
	command := model.applyEvent(testProtocolEvent(model.session.ThreadID, "turn-1", protocol.TurnCompleteEvent{
		Status: protocol.TurnStatusCompleted, Outcome: protocol.TurnOutcomeCompleted, FinishedAt: time.Now().UTC(),
	}))
	message := command().(userMessageAdmittedMsg)
...
func TestTUIQueuedModeMismatchRestoresComposer(t *testing.T) {
	_, model := newTestModel(t, nil)
	model.running = true
	if err := model.nextTurnQueue.Enqueue(queuedInput("planned next", protocol.ModeKindPlan, model.session.ThreadID, model.session.Generation)); err != nil {
		t.Fatal(err)
	}
...
func TestTUIUnexpectedQueuedSteerStopsDrainWithoutDuplicateRestore(t *testing.T) {
	_, model := newTestModel(t, nil)
	fake := fakeApplication(t, model)
...
func TestTUIQueuedPlanInputSuppressesImplementationOverlay(t *testing.T) {
	...
	if err := model.nextTurnQueue.Enqueue(queuedInput("refine the plan", protocol.ModeKindPlan, model.session.ThreadID, model.session.Generation)); err != nil {
		t.Fatal(err)
	}
	command := model.applyEvent(testProtocolEvent(model.session.ThreadID, "turn-1", protocol.TurnCompleteEvent{
		Status: protocol.TurnStatusCompleted, Outcome: protocol.TurnOutcomeCompleted, FinishedAt: time.Now().UTC(),
...
func TestTUIAttachmentReplacementClearsQueuedInput(t *testing.T) {
	_, model := newTestModel(t, nil)
	if err := model.nextTurnQueue.Enqueue(queuedInput("old thread", protocol.ModeKindDefault, model.session.ThreadID, model.session.Generation)); err != nil {
		t.Fatal(err)
	}
...
func TestTUIStaleQueuedSubmissionResultCannotRestoreIntoNewAttachment(t *testing.T) {
	_, model := newTestModel(t, nil)
	oldThreadID := model.session.ThreadID
	if err := model.nextTurnQueue.Enqueue(queuedInput("old thread", protocol.ModeKindDefault, oldThreadID, model.session.Generation)); err != nil {
		t.Fatal(err)
	}
	...
	model.handleUserMessageRejection(userMessageRejectedMsg{submission: submission, err: errors.New("late")})
...
func TestQueuedInputPreviewYieldsToPopupAndOverlay(t *testing.T) {
	_, model := newTestModel(t, nil)
	if err := model.nextTurnQueue.Enqueue(queuedInput("next", protocol.ModeKindDefault, model.session.ThreadID, model.session.Generation)); err != nil {
		t.Fatal(err)
	}
