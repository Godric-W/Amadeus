...
默认 execute Run 使用 Plan-guided ReAct：模型可按任务复杂度调用 `update_plan` 维护软计划，但始终由同一个 Think → Analyze → Act → Observe Reactor 执行。输入 `/plan` 会把 Composer 切换到只规划不实施的 Plan Mode，下一条普通输入使用同一 Reactor 和只读工具集。当前不内置 RAG、不自动推断长期用户记忆，也不启用 Multi-Agent 主链。
...
交互模式每次提交创建一个独立 Run。输入 `/exit`、空输入时按 Ctrl+D，或向 Plain 模式发送 EOF，可以退出；按 Ctrl+C 会将当前 Run 标记为 interrupted、补齐悬空 Tool 协议并停止运行。后续输入创建新 Run，模型从 canonical rollout 看到中断事实并重新规划，不恢复旧 Go 调用栈。
