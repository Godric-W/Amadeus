...
func TestModelClientSessionRetriesDroppedStreamAndReplacesAttemptDraft(t *testing.T) {
	...
	drafts := make(map[protocol.ItemID]string)
	for _, event := range events {
		...
		case protocol.ItemStartedEvent:
			drafts[message.Item.ID] = message.Item.Text
		case protocol.AgentMessageContentDeltaEvent:
			if message.Reset {
				assistantResetEvents++
				drafts[message.ItemID] = message.Delta
			} else {
				drafts[message.ItemID] += message.Delta
			}
	...
	if draft := drafts["sample-1:assistant"]; draft != "recovered" {
		t.Fatalf("assistant draft was not replaced: %q", draft)
	}
	if draft := drafts["sample-1:reasoning"]; draft != "new reasoning" {
		t.Fatalf("reasoning draft was not replaced: %q", draft)
	}
}
