...
- Footer/Statusline 对齐 Codex StatusLineAccent 的 theme-first 规则：TrueColor/ANSI256 根据终端明暗背景选择 Catppuccin Mocha/Latte，并从对应语义 token 解析 model/type、path/string、branch/function、usage/number、mode/keyword 与 thread/heading 色，再应用 Codex 85% saturation softening；ANSI16 使用 Codex cyan/green/magenta fallback，No Color 整行 dim。分隔符使用 dim；context 达到警告/失败阈值后覆盖为 yellow/red。固定六个 Amadeus StatusLineItem 先组成完整左侧 statusline，窄宽度时从右侧截断并追加`…`；Plan mode 使用独立右列并按可用宽度在完整 `Plan mode (shift+tab to cycle)` 与 `Plan mode` 之间收缩，Default mode 不显示模式标签。ContextUsed/ContextWindowSize 属于左侧 statusline，按整行截断策略参与显示。
