...
`)
	...
	flags := &configFlags{}
	command := newRootCommandWithOptions(flags, RootOptions{Environment: bootstrap.Environment{
		AmadeusRootError: errors.New("root unavailable"),
...
`)
	...
	var output bytes.Buffer
	command := newRootCommandWithOptions(&configFlags{}, RootOptions{Environment: bootstrap.Environment{
