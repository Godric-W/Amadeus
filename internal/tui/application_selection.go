...
func (model appModel) resolveApproval(decision policy.ApprovalDecision) (tea.Model, tea.Cmd) {
	...
	if prompt != nil {
		...
		commands = append(commands, func() tea.Msg {
			if err := model.app.options.Application.ResolveApproval(model.ctx, requestID, decision); err != nil {
				return operationFailedMsg{operation: "resolve approval", err: err}
			}
			return nil
		})
	}
	if model.running {
		commands = append(commands, workingTick())
	}
	return model, tea.Batch(commands...)
}

func (model appModel) handleSelectionKey(key tea.KeyMsg) (tea.Model, tea.Cmd) {
...
func (model appModel) renderSelectionOverlay(width int) string {
