...
func TestToolsListShowsTargetExposureAndMigrationMetadata(t *testing.T) {
	...
	if len(lines) != 18 || !strings.Contains(lines[0], "EXPOSURE") || !strings.Contains(lines[0], "CONDITION") {
		t.Fatalf("unexpected tools list shape: %q", output.String())
	}
	wantNames := []string{"apply_patch", "execute_command", "glob_files", "grep_code", "list_dir", "read_file", "request_permissions", "update_plan", "write_stdin", "view_image", "read_skill", "web_search", "web_fetch", "mcp_list_tools", "mcp_call", "mcp_list_resources", "mcp_read_resource"}
