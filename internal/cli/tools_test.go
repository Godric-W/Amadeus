...
func TestToolsListShowsTargetExposureAndMetadata(t *testing.T) {
	...
	if len(lines) != 22 || !strings.Contains(lines[0], "EXPOSURE") || !strings.Contains(lines[0], "CONDITION") {
		t.Fatalf("unexpected tools list shape: %q", output.String())
	}
	wantNames := []string{"read", "edit", "write", "glob", "grep", "execute_command", "update_plan", "request_user_input", "write_stdin", "spawn_agent", "send_input", "wait_agent", "close_agent", "view_image", "read_skill", "web_search", "web_fetch", "mcp_list_tools", "mcp_call", "mcp_list_resources", "mcp_read_resource"}
	...
	wantMetadata := map[string][]string{
		...
		"spawn_agent": {"conditional", "multi_agent.enabled", "available", "none"},
