...
func multiAgentSpec(name string) tool.ToolSpec {
	const spawnGuidance = `Spawn a read-only explorer sub-agent for one concrete, bounded, self-contained side task. First identify the critical path and delegate only independent exploration that materially advances the main task. Do not delegate work needed for your immediate next action, duplicate already delegated work, or spawn repeatedly for one unresolved task. Continue useful non-overlapping local work after spawning. Spawn multiple independent investigations in parallel when appropriate, and use wait_agent only when the result blocks the next critical-path step. The child cannot modify files, execute commands, request user input, or spawn agents.`
	specs := map[string]tool.ToolSpec{
		...
		"wait_agent":  {Name: "wait_agent", Description: "Wait for one or more sub-agents to leave pending or running state. Use sparingly: completed agents also send a bounded parent notification, so wait only when the result blocks the next step. A timeout returns current status without closing agents.", InputSchema: json.RawMessage(`{"type":"object","properties":{"ids":{"type":"array","minItems":1,"description":"Agent thread IDs to observe.","items":{"type":"string","minLength":1}},"timeout_ms":{"type":"integer","minimum":10000,"maximum":3600000,"description":"Wait timeout in milliseconds. Defaults to 30000."}},"required":["ids"],"additionalProperties":false}`), SideEffect: tool.SideEffectNone},
