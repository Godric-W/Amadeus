...
func (model *appModel) handleAppEventNow(event application.InteractiveEvent) tea.Cmd {
	...
	case application.ApprovalRequested:
		...
		model.approval = &approvalState{requestID: event.RequestID, request: event.Request}
...
func (model *appModel) clearInteractiveState() {
...
func (model *appModel) openSessions(sessions []application.SessionOption) {
	if len(sessions) == 0 {
...
func (model *appModel) openSkills(skills []application.SkillOption) {
