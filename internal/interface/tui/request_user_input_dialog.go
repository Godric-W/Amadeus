...
func (model fullscreenModel) handleRequestUserInputKey(key tea.KeyMsg) (tea.Model, tea.Cmd) {
...
func (model fullscreenModel) advanceUserInputDialog() (tea.Model, tea.Cmd) {
	...
	return model, func() tea.Msg {
		if err := model.app.options.Application.ResolveUserInput(context.WithoutCancel(model.ctx), requestID, response); err != nil {
			return fullscreenOperationFailedMsg{operation: "resolve user input", err: err}
		}
		return nil
	}
}

func (model fullscreenModel) renderRequestUserInputDialog(width int) string {
	request, dialog := model.userInputRequest, model.userInputDialog
	if request == nil || dialog == nil || dialog.question >= len(request.Questions) {
		return ""
	}
